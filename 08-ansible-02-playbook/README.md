...
Через playbook vars можно задать следующие параметры:
- версии устанавливаемых приложений;
- имя базы данных для хранения логов;
- структуру таблицы для хранения логов;
- содержимое конфигурационного файла для приложения `vector`;
...
- `vector_config` производит изменение в конфиге `vector`;
- `drop_clickhouse_database_logs` удаляет базу данных (по умолчанию не выполняется);

Таким образом для полной установки инфраструктуры указывать тэги не требуется.
